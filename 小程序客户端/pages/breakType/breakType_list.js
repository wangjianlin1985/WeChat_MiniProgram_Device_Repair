...
Page({
  ...
  // 加载故障类型列表
  listBreakType: function () {
    var self = this
    var url = config.basePath + "api/breakType/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          breakTypes: self.data.breakTypes.concat(res.data.list),
  ...
  //删除故障类型记录
  removeBreakType: function (e) {
    var self = this
    var breakTypeId = e.currentTarget.dataset.breaktypeid
    wx.showModal({
      title: '提示',
      content: '确定要删除breakTypeId=' + breakTypeId + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/breakType/delete/" + breakTypeId
          utils.sendRequest(url, null, function (res) {
            ...
            //删除故障类型后客户端同步删除数据
            var breakTypes = self.data.breakTypes;
            for (var i = 0; i < breakTypes.length; i++) {
              if (breakTypes[i].breakTypeId == breakTypeId) {
                breakTypes.splice(i, 1)
                break
              }
            }
            self.setData({ breakTypes: breakTypes })
          })
        } else if (sm.cancel) {
          console.log('用户点击取消')
        }
