...
Page({
  ...
  // 加载任务状态列表
  listTaskInfoState: function () {
    var self = this
    var url = config.basePath + "api/taskInfoState/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          taskInfoStates: self.data.taskInfoStates.concat(res.data.list),
  ...
  //删除任务状态记录
  removeTaskInfoState: function (e) {
    var self = this
    var stateId = e.currentTarget.dataset.stateid
    wx.showModal({
      title: '提示',
      content: '确定要删除stateId=' + stateId + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/taskInfoState/delete/" + stateId
          utils.sendRequest(url, null, function (res) {
            ...
            //删除任务状态后客户端同步删除数据
            var taskInfoStates = self.data.taskInfoStates;
            for (var i = 0; i < taskInfoStates.length; i++) {
              if (taskInfoStates[i].stateId == stateId) {
                taskInfoStates.splice(i, 1)
                break
              }
            }
            self.setData({ taskInfoStates: taskInfoStates })
          })
        } else if (sm.cancel) {
          console.log('用户点击取消')
        }
